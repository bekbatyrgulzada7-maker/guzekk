...
<button onclick="savePDF('JoyStone')">🏖️ JoyStone PDF</button>
...
function render(){
    ...
    types.forEach(t=>{
        ...
        section.innerHTML=`
            <h2 onclick="toggle('${t.name}')">${t.name}</h2>

            <div id="${t.name}" style="display:block;">
                <div class="grid">

                ${data[t.name].map((c,i)=>{

                    let status=c.status || "free";

                    return `
                    <div class="cell ${status}" onclick="openModal('${t.name}',${i})">
                        <b>${i+1}</b><br>
                        ${c.room || "-"}<br>
                        ${c.emoji || ""}
                    </div>`;
